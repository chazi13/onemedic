    ...
    <div class="card-body">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group row">
                        ...
                        <div class="col-lg-7">
                        <?php echo form_dropdown('lokasi_barang_id_dari', $optionsLokasiBarang, $lokasiDari, 'class="form-control"'); ?>
                        </div>
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group row">
                        <label class="col-lg-3 control-label">Dari Lokasi</label>
                        <div class="col-lg-7">
                        <?php echo form_dropdown('lokasi_barang_id_ke', $optionsLokasiBarang, $lokasiKe, 'class="form-control"'); ?>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="table-responsive">
            <table class="display table table-bordered table-striped" id="dynamic-table-ajax-source">
                <thead>
                    <tr>
                        <td class="font-weight-bold">Tgl. mutasi</td>
                        <td class="font-weight-bold">Barang / Aset</td>
                        <td class="font-weight-bold">Kondisi</td>
                        <td class="font-weight-bold">Dari Lokasi</td>
                        <td class="font-weight-bold">Ke Lokasi</td>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    ?>
                    <tr>
                        <td class="text-right" style="width:150px;" ><?php echo $this->utility->mysql_to_tanggal($row->tanggal_mutasi); ?></td>
                        <td><?php echo $row->barang_kode . ' ' .$row->barang_nama ; ?></td>
                        <td><?php echo $row->kondisi_nama ; ?></td>
                        <td><?php echo $row->lokasi_barang_nama_dari ; ?></td>
                        <td><?php echo $row->lokasi_barang_nama_ke ; ?></td>
                    </tr>
                    <?php
                        endforeach;
                    endif;
...
<script>
    $(function() {
        ...
        function loadLaporan(){
            var jenis = $("select[name=jenis_barang_id]").val() || 0;
            var dari = $("select[name=lokasi_barang_id_dari]").val() || 0;
            var ke = $("select[name=lokasi_barang_id_ke]").val() || 0;
            var tglAwal = $("input[name=tgl_awal]").val();
            var tglAkhir = $("input[name=tgl_akhir]").val();
            window.location = '<?php echo site_url("laporan/mutasi_barang/index/")?>'+tglAwal+'/'+tglAkhir+'/'+jenis+'/'+dari+'/'+ke;
        }
    });
</script>
