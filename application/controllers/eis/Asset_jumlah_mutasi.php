...
class Asset_jumlah_mutasi extends Admin_Controller {
    ...
    function sourcedata() {
        ...
        $rows = $this->db->query("
                    SELECT
                        SUBSTRING(tanggal_mutasi::TEXT,1,7) AS bulan,
                        COUNT(id) AS jumlah
                    FROM
                        asset_mutasi
                    WHERE status = '1' " . $kodeFilter . " AND tanggal_mutasi >= '".$bulanLalu."-01'
        ...
        $data = array(
            'title' => 'Grafik mutasi Aset Dari Tanggal 01-'.date('m-Y', strtotime('-12 months', strtotime($bulanIni))).' s/d '.date('Y-m-d'),
            'labels' => $arrLabels,
            'values' => $arrValues,
        );
