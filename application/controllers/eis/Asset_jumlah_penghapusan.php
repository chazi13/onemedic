...
class Asset_jumlah_penghapusan extends Admin_Controller {
    ...
    function sourcedata() {
        ...
        $rows = $this->db->query("
                    SELECT
                        SUBSTRING(tanggal_penghapusan::TEXT,1,7) AS bulan,
                        COUNT(id) AS jumlah
                    FROM
                        asset_penghapusan
                    WHERE status = '1' " . $kodeFilter . " AND tanggal_penghapusan >= '".$bulanLalu."-01'
                    GROUP BY tanggal_penghapusan
                    ORDER BY
                    tanggal_penghapusan ASC
            ")->result();
        $arrLabels = array();
        $arrValues = array();
        if($rows){
        ...
        $data = array(
            'title' => 'Grafik penghapusan Aset Dari Tanggal 01-'.date('m-Y', strtotime('-12 months', strtotime($bulanIni))).' s/d '.date('Y-m-d'),
            'labels' => $arrLabels,
            'values' => $arrValues,
        );
