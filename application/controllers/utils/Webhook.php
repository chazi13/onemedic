    ...
    public function index()
    {
        ...
        if (!empty($payload)) {
            ...
            try {
                ...
                exec("composer install -d " . realpath(FCPATH) . DIRECTORY_SEPARATOR . " 2>&1", $composer_output);
                $this->log("Updating composer... " . implode("\n", $composer_output));

                // Migrate database
                $migration_output = array();
                exec("php index.php cli database migrate 2>&1", $migration_output);
                $this->log("Migrating database... " . implode("\n", $migration_output));

                // Update bower
                $bower_output = array();
                chdir('assets');
                exec("bower install 2>&1", $bower_output);
                $this->log("Updating bower... " . implode("\n", $bower_output));
