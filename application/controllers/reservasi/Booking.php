...
class Booking extends Admin_Controller {
    ...
    function __construct() {
		...
		if ($this->input->post('cancel-button'))
            redirect('reservasi/booking');
    }

    public function index()
    {
        
        $this->_update_no_urut_daftar(date("Y-m-d", strtotime("-1 day"))); 
        $optionsPoli = $this->reservasi_poli_model->get_dropdown_array('poli_nama', 'id');
        $arrParamShiftPagi =  array('status' => 1, 
                                    'tanggal_kunjungan' => date('Y-m-d'));
 
        $rowsShiftPagi = $this->db
                    ->order_by('dokter_praktek_mulai, no_urut_daftar','ASC')
                    ->get_where('pdf_reservasi', $arrParamShiftPagi)
                    ->result();
                    
        $this->data['optionsPoli'] = $optionsPoli;
        $this->data['rowsShiftPagi'] = $rowsShiftPagi;
        $this->template
            ->set_title('Booking')
            ->set_layout('pendaftaran')
            ->build('reservasi/booking/index', $this->data);
    }
    ...
    public function history(){
        ...
        $this->template
            ...
            ->set_js('libs/DataTables/extensions/ColVis/dataTables.colVis.min')
            ->set_js('libs/DataTables/extensions/TableTools/dataTables.tableTools.min')
            ->set_js('libs/bootstrap-datepicker/bootstrap-datepicker')
    ...
    function _update_no_urut_daftar($date){
        $rowsPraktek = $this->db->query("SELECT DISTINCT poli_id, poli_nama, dokter_praktek_mulai FROM pdf_reservasi WHERE created::DATE = '".$date."' ORDER BY poli_nama, dokter_praktek_mulai ASC")->result();
        if($rowsPraktek){
            foreach($rowsPraktek as $rowPraktek){
                $rowsPasien = $this->db->query("SELECT id FROM pdf_reservasi WHERE poli_id = ".$rowPraktek->poli_id." AND dokter_praktek_mulai = '".$rowPraktek->dokter_praktek_mulai."' AND created::DATE = '".$date."' ORDER BY poli_nama, dokter_praktek_mulai, created ASC")->result();
                if($rowsPasien){
                    $i = 1; 
